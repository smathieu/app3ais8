...
netn.trainParam.show = 50;       % Frequency of progress displays (in epochs).
...
RNAoutput = sim(net,lettre);
RNAoutputn = sim(netn,lettre);

[lettreBrut maximum]= max(RNAoutput);
[lettreBrutn maximumn]= max(RNAoutputn);

% Lettre avec le reseau entraine sans bruit
lettreNet = setstr(maximum+64)

% Lettre avec le reseau entraine avec bruit
lettreNetn = setstr(maximumn+64)
